...
Намерете броя на символите в /etc/passwd. А колко реда има в /etc/passwd?
	cat /etc/passwd | wc -m		||--chars		///брой символи
	cat /etc/passwd | wc -l		||--lines		//брой редове
...
- първите 26 символа
	cat /etc/passwd | head -c 26	// за верификация | wc --chars
...
- 151-я ред (или друг произволен, ако нямате достатъчно редове)
	cat /etc/passwd | head -n 154 | tail -n 1		//за верификация  grep -n 'нещо от тоя ред' /etc/passwd

- последните 4 символа от 13-ти ред
	cat /etc/passwd | head -n 13 | tail -n 1 | tail -n 5

-- 03-a-2100
Отпечатайте потребителските имена и техните home директории от /etc/passwd.
	cut -d ':' -f 5,6 /etc/passwd | tr

	//малко по-различна задача : ако са промелнив брой запетайки
	cat /etc/passwd | cut -d ':' -f 5,6 | sed -r 's/^([[:alpha:] ]+),.*:(.*)$/\1 \2 /g'	regex : ^([[:alpha:] ]+),.*:(.*)$
	//без една запетая
	cat /etc/passwd | cut -d ':' -f 5,6 | sed -r 's/^([[:alpha:] ]+).*:(.*)$/\1 \2 /g'
...
Сортирайте създадения файла по второ поле (numeric, alphabetically).
	ls -l | tail -n +2 > out.txt
	sort -n -t ' ' -k 2 out.txt	|| sort -t ' ' -k 2 out.txt

-- 03-a-5000
Отпечатайте 2 реда над вашия ред в /etc/passwd и 3 реда под него // може да стане и без пайпове
	grep -B 2 -A 3 's62167' /etc/passwd

-- 03-a-5001
Колко хора не се казват Ivan според /etc/passwd
	cat /etc/passwd | cut -d : -f 5 		=> ще даде 2/3 имена, но и спец, курс, група
	cat /etc/passwd | awk -F [:,] '{print $5}' => дава направо двете или трите имена и само тях, защото ползва и втория подаден разделител ,

	cat /etc/passwd | awk -F [:,] '{print $5'}' | еgrep -v '^Ivan'
	//или
	cat /etc/passwd | cut -d : -f 5 | еgrep -v '^Ivan'

-- 03-a-5002
Изведете имената на хората с второ име по-дълго от 7 (>7) символа според /etc/passwd
	cat /etc/passwd | awk -F [:,] '{print $5}' | awk -F ' ' '{print $2}' | egrep "[[:alpha:]]{8,}"		"[a-Z]{8,}" <=> "[a-zA-Z]{8,}"

-- 03-a-5003
Изведете имената на хората с второ име по-късо от 8 (<=7) символа според /etc/passwd // !(>7) = ?
	cat /etc/passwd | awk -F [:,] '{print $5}' | awk -F ' ' '{print $2}' | egrep -v "[a-zA-z]{8,}" 		|| или egrep "^[[:alpha:]]{,7}$"

-- 03-a-5004
Изведете целите редове от /etc/passwd за хората от 03-a-5003
	??
	cat /etc/passwd | awk -F [:,] '{print $5}' | awk -F ' ' '{print $2}' | egrep -v "[a-zA-z]{8,}" | xargs -I'{}' egrep '{}' /etc/passwd

-- 03-b-0300
Намерете факултетния си номер във файлa /etc/passwd.
	cat /etc/passwd | grep -o "Ivan Chuchulski" | cut -d : -f 1 | tail -c +2
...
/home/s...../dir1/file3
	...
	mkdir -p /home/SI/s62167/dir1/file{1,2,3}
...
Във file2 подменете всички малки букви с главни.
	cat file2 | sed -r 's/[a-z]/[A-Z]/g'	или	 cat file2 | tr [a-z] [A-Z]
-- 03-b-4002
Във file3 изтрийте всички "1"-ци.
	cat file3 | sed -r 's/1//g'								cat file3| tr -d "1"
...
Отпечатайте правата (permissions) и имената на всички файлове, до които имате read достъп, намиращи се в директорията /tmp.
	find /tmp -maxdepth 1 -type f -readable -printf "%m %f\n"
